 ...
 *
 *  3) 어드바이스(Advice)
 *      - 횡단 광심에 해당하는 공통의 부가기능 메서드
 *
 *  4) 애스팩트(Aspect)
 *      - 포인트컷과 어드바이스의 결합된 모듈 형태
 *
 *  5) 위빙(Weaving)
 *      - 포인트컷(핵심 관심)이 실행될 때 어드바이스가 포인트컷에 결합되는 과정
 *
 * 포인트컷 표현식
 ...
 *      - kr.co.cho3.. : 해당 패키지로 시작하는 모든 패키지 대상
 *      - kr.co.ch03..service : 해당 패키지로 시작해서 마지막 패키지 명이 service 로 끝나는 패키지 대상
 *
 *  3) 클래스명
 ...
 *      - *(*) : 매개변수를 1개 갖는 모든 메서드
 *      - *(*,*) : 매개변수를 2개 갖는 모든 메서드
 *      - get*() : 매개변수가 없고 메서드 이름이 get 으로 시작하는 모든 메서드
 */
public class App 
{
